...
int sqlite3VdbeExec(
  ...
  printf("> Executing opcodes contained at Vdbe *p\n");
  printf(">> Number of opcodes at the VM: %d \n", p->nOp);

  for(int i=0; i<p->nOp; i++) {
    VdbeOp *op = &p->aOp[i];
    printf(">>> Opcode %d: %s | p1=%d p2=%d p3=%d p4=%p p5=%d \n", 
             i, 
	     sqlite3OpcodeName(op->opcode),
	     op->p1, op->p2, op->p3, op->p4.p, op->p5);
